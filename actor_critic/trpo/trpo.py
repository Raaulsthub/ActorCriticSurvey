...
def train(epochs=1000, num_rollouts=10, render_frequency=None):
    mean_total_rewards = []
    global_rollout = 0
    best_score = -1000
    ...
    for epoch in range(epochs):
        ...
        if best_score < np.mean(mean_total_rewards[-20:]):
            best_score = np.mean(mean_total_rewards[-20:])
            save_checkpoint(actor, 'actor')
            save_checkpoint(critic, 'critic')
            print('saved_chkpt')
    ...
    data = {'episode': np.arange(len(mean_total_rewards)), 'avg_reward': mean_total_rewards}
    df = pd.DataFrame(data)
    df.to_csv('trpo_lunar_lander.csv', index=False)
